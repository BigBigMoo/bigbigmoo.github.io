...
var logId = null;
var pageUrl = null;
var global_loginName=null;
var global_userPwd=null;
var init=function(){
    //初始化
...
var memberLoginName = function (handler){
    ...
    $.ajax({
     ...
     //   url		:  global_backURL + "/reception/member/front-member!getLoginName." + suffix,
        url		:  global_backURL + "/reception/member/front-member!getUserName." + suffix,
        dataType:  suffix,
        success : function(data){
            debugger;
			/*
            ...
            if(data == ""){
                var url=window.location.href;
                window.location.href=url.substring(0,url.lastIndexOf("\/mb")+3)+"/login.html";
            }else{
                var datas=data.split("@");
                $("#memberUserName").html("当前用户："+datas[1]+" <a href='javascript:;' onclick='window.close()' style='color:black;'>退出</a>");
                global_loginName=datas[0];
                global_userPwd=datas[2];
            //    $("#memberLoginName").html(datas[0]);
            //    $("#memberPwd").html(datas[2]);
            }
            if(handler){
                handler(data);
            }
